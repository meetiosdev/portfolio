(function () {
  ...
  // 1. Enforce static admin auth check
  ...
  async function initErrorsDashboard() {
    try {
      ...
      console.error('Failed to fetch error logs from Supabase:', e);
      document.getElementById('errors-container').innerHTML = `
        <div class="empty-state glass-card">
          <h2>Failed to load logs</h2>
          <p>Unable to connect directly to the Supabase database. Try refreshing in a few moments.</p>
        </div>
      `;
    } finally {
  ...
  function renderErrors(events) {
    ...
    errorEvents.forEach(function (errorEvent, idx) {
      ...
      // Assemble timeline step HTML elements
      let lastTime = null;
      const timelineHtml = sessionEvents.map(function (step, stepIdx) {
        ...
        if (stepName === 'error') {
          ...
          descHtml = `<span style="color: var(--rose); font-weight: 700;">Crashed!</span> JavaScript runtime exception.`;
